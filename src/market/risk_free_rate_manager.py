...
Manages U.S. Treasury rates for calculating excess returns in factor models.

Classes:
- RiskFreeRateBuilder: Fetches data from FRED API, builds, and saves to cache
- RiskFreeRateManager: Loads cached data and provides utility functions (no API key needed)
...
class RiskFreeRateManager:
    """
    Load and manage cached risk-free rate data.

    Use this class when you just need to load existing cached data.
    No API key required.
    """
    ...
    def __init__(
        ...
        Args:
            data_root: Root directory for cached risk-free rate data
    ...
    def load_risk_free_rate(
        ...
        if not cache_path.exists():
            raise FileNotFoundError(
                f"Risk-free rate cache not found: {cache_path}\n"
                f"Use RiskFreeRateBuilder to fetch and save data first."
            )
        ...
        if len(df) == 0:
            raise ValueError(
                f"No data in requested date range [{start_date}, {end_date}] in cache.\n"
                f"Use RiskFreeRateBuilder to fetch data for this date range."
            )
    ...
    def calculate_excess_returns(
        ...
        rf_returns = self.calculate_risk_free_returns(
            dates=dates, rate_type=rate_type, frequency=frequency
        )
