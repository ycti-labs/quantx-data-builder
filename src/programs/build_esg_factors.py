...
- Risk-free rate normalization (explicit frequency/percentage config)
- Save to data/results/esg_factors/esg_factors.parquet

Usage:
    # Quick test with specific tickers (equal-weighted)
    ...
    # Value-weighted factors (market-cap weighted)
    python src/programs/build_esg_factors.py --continuous-esg-only --weighting value

    # Sector-neutral with custom quantile
    python src/programs/build_esg_factors.py --sector-neutral --weighting value --quantile 0.3

    # We use
    python src/programs/build_esg_factors.py --continuous-esg-only --sector-neutral --quantile 0.2 --start-date 2014-01-01 --end-date 2024-12-31

    # Custom date range and RF configuration
...
def load_esg_panel(
    ...
    for i, ticker in enumerate(tickers, 1):
        if i % 50 == 0:
            logger.info(
                f"  Progress: {i}/{len(tickers)} ({success_count} success, {fail_count} failed)"
            )
...
def load_price_panel(
    ...
    Returns:
        DataFrame: MultiIndex [date, ticker], columns ['adj_close', 'adj_volume']
    """
    ...
    for i, ticker in enumerate(tickers, 1):
        ...
        try:
            ticker_path = (
                data_root / "curated" / "tickers" / "exchange=us" / f"ticker={ticker}"
            )
            ...
            for year_dir in sorted(price_dir.glob("year=*")):
                ...
                if parquet_file.exists():
                    ...
                    if not df.empty:
                        # Include adj_volume for market cap weighting
                        ticker_prices.append(df[["date", "adj_close", "adj_volume"]])
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["equal", "value"],
        default="equal",
        help="Portfolio weighting: 'equal' (academic) or 'value' (market-cap weighted)",
    )
    parser.add_argument(
        "--value-weighted",
        action="store_true",
        help="(DEPRECATED: use --weighting value) Use value-weighted portfolios",
    )
    ...
    parser.add_argument(
        ...
        default="monthly",
        help="Risk-free rate frequency (default: monthly)",
    )
    parser.add_argument(
        "--rf-is-percent",
        action="store_true",
        help="Whether RF is in percentage points (e.g., 5.0 = 5%%)",
    )
    ...
    if args.debug:
        ...
        # Also set debug for ESG factor builder
        logging.getLogger("esg.esg_factor").setLevel(logging.DEBUG)
    ...
    # Handle deprecated --value-weighted flag
    if args.value_weighted:
        weighting = "value"
        logger.warning("--value-weighted is deprecated, use --weighting value instead")
    else:
        weighting = args.weighting

    logger.info(f"Weighting: {weighting}")
    logger.info(f"RF Frequency: {args.rf_frequency}")
    logger.info(f"RF Is Percent: {args.rf_is_percent}")
    if args.dry_run:
    ...
    if args.max_tickers and args.max_tickers < len(tickers):
        ...
        logger.info(
            f"Limited to {len(tickers)} tickers (--max-tickers {args.max_tickers})"
        )
    ...
    # Load sector mapping if sector-neutral is requested
    sector_map = None
    if args.sector_neutral:
        logger.info("")
        logger.info("Loading sector classifications for sector-neutral ranking...")
        try:
            # Load from saved ESG Parquet files (more reliable than raw Excel)
            sector_map = esg_mgr.get_sector_mapping(tickers, exchange="us")
            if sector_map is not None and not sector_map.empty:
                logger.info(f"Loaded sector data for {len(sector_map)} tickers")
                logger.info(f"Sectors found: {sector_map.unique().tolist()}")
            else:
                logger.warning(
                    "Sector-neutral ranking requested but no sector data available. "
                    "Falling back to cross-sectional ranking."
                )
                args.sector_neutral = False
        except AttributeError:
            logger.warning(
                "ESGManager does not support sector mapping. "
                "Sector-neutral ranking will fall back to cross-sectional ranking."
            )
            args.sector_neutral = False
    ...
    factor_df = factor_builder.build_factors(
        ...
        save=True,
        save_legs=True,  # Save detailed long/short leg returns
    )
    ...
    # Load and display leg returns
    legs_df = factor_builder.load_factor_legs()
    if legs_df is not None:
        print("\n" + "=" * 80)
        print("FACTOR LEG RETURNS (Long/Short Breakdown)")
        print("=" * 80)

        # NOTE: Leg returns are monthly decimal returns (e.g., -0.01 = -1% that month)
        # Calculate leg statistics
        leg_summary = pd.DataFrame(
            {
                "Mean (Monthly %)": legs_df.mean()
                * 100,  # Convert to monthly percentage
                "Std (Monthly %)": legs_df.std() * 100,  # Convert to monthly percentage
                "Mean (Ann. %)": legs_df.mean() * 12 * 100,  # Annualized
                "Std (Ann. %)": legs_df.std() * np.sqrt(12) * 100,  # Annualized
                "Sharpe": (legs_df.mean() / legs_df.std()) * np.sqrt(12),
            }
        )
        ...
        print("\nRecent Leg Returns (last 5 months, monthly returns in %):")
        print((legs_df.tail(5) * 100).to_string())  # Convert to percentage

        # Show average long vs short performance
        print("\n" + "=" * 80)
        print("AVERAGE LONG VS SHORT PERFORMANCE")
        print("=" * 80)
        for factor_name in ["ESG", "E", "S", "G", "ESG_mom_z"]:
            long_col = f"{factor_name}_long"
            short_col = f"{factor_name}_short"
            factor_col = f"{factor_name}_factor"

            if long_col in legs_df.columns and short_col in legs_df.columns:
                long_mean_monthly = legs_df[long_col].mean() * 100  # Monthly percentage
                short_mean_monthly = (
                    legs_df[short_col].mean() * 100
                )  # Monthly percentage
                factor_mean_monthly = (
                    legs_df[factor_col].mean() * 100
                )  # Monthly percentage

                long_mean_annual = (
                    legs_df[long_col].mean() * 12 * 100
                )  # Annualized percentage
                short_mean_annual = (
                    legs_df[short_col].mean() * 12 * 100
                )  # Annualized percentage
                factor_mean_annual = (
                    legs_df[factor_col].mean() * 12 * 100
                )  # Annualized percentage

                print(f"\n{factor_name}:")
                print(f"  Long Leg (monthly):    {long_mean_monthly:>7.2f}%")
                print(f"  Short Leg (monthly):   {short_mean_monthly:>7.2f}%")
                print(f"  Factor (monthly):      {factor_mean_monthly:>7.2f}%")
                print(f"  Long Leg (annual):     {long_mean_annual:>7.2f}%")
                print(f"  Short Leg (annual):    {short_mean_annual:>7.2f}%")
                print(f"  Factor (annual):       {factor_mean_annual:>7.2f}%")

    # Save location
    factors_file = factor_builder.factors_dir / "esg_factors.parquet"
    legs_file = factor_builder.factors_dir / "esg_factor_legs.parquet"
    logger.info("")
    logger.info(f"✅ Saved factors to: {factors_file}")
    logger.info(f"✅ Saved leg returns to: {legs_file}")
